...
export default function Dashboard() {
  ...
  // Lógica para marcar o intervalo no calendário e calcular legenda anterior
  useEffect(() => {
    let marked = {};
    if (dataInicio) {
  ...
  const CardResumo = ({ cor, emoji, titulo, valor, subtitulo, diferenca, isMoeda, mostrarToggle, mostrarLegendaAnterior }) => (
    <View style={styles.cardResumo}>
      <View style={[styles.barraStatus, { backgroundColor: cor }]} />
      <View style={styles.cardInfo}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Text style={styles.cardTitulo}>{emoji} {titulo}</Text>
          {mostrarToggle && (
  ...
  return (
    <View style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
          ...
          >
            ...
            <View style={styles.textoFiltroContainer}>
              ...
              <Text style={styles.txtDataAtual}>
                {formatarPT(dataInicio)} — {formatarPT(dataFim || dataInicio)}
              </Text>
            </View>
        ...
        <CardResumo
          cor="#6f6df1" emoji="💰" titulo="Faturamento Estimado"
          valor={lucroBruto.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          diferenca={diferencaBruto} isMoeda={true}
          subtitulo="em relação ao período anterior"
          mostrarLegendaAnterior={true}
        />

        <CardResumo
          cor="#41c482" emoji="📉" titulo="Vendas Realizadas"
          valor={`${quantidadeTotalVendida} vendas`}
          diferenca={diferencaVenda} isMoeda={false}
          subtitulo="em relação ao período anterior"
          mostrarLegendaAnterior={true}
        />

        <CardResumo
          cor="#f0b731" emoji="💵" titulo="Lucro Bruto"
          valor={lucroLiquido.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          subtitulo={`${liquidoMercadoria.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} em produtos`}
        />

        <CardResumo
          cor="#C60018"
          emoji={exibirPratos ? "🍽️" : "📦"}
          titulo={exibirPratos ? "Prato Mais Vendido" : "Produto Mais Vendido"}
          valor={exibirPratos ? pratoMaisVendido.nome : produtoMaisVendido.nome}
          subtitulo={`${exibirPratos ? pratoMaisVendido.quantidadeVendida : produtoMaisVendido.quantidadeVendida} unidades vendidas`}
          mostrarToggle={true}
        />
        ...
        <View style={styles.secaoBranca}>
          ...
          {(exibirPratos ? dadosPratos : dadosProdutos).slice(0, 7).map((item, index, arr) => (
            <View key={index} style={[styles.itemRanking, index === arr.length - 1 && { borderBottomWidth: 0 }]}>
              ...
              <Text style={styles.qtdItem}>{item.quantidadeVendida}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  btnFiltroModerno: {
    flexDirection: 'row', backgroundColor: '#fff', padding: 10, borderRadius: 16,
    alignItems: 'center', elevation: 4, shadowColor: '#1E22AA', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.1, shadowRadius: 8, borderWidth: 1, borderColor: '#e0e0ff',
  },
  iconeCirculo: {
    backgroundColor: '#1E22AA', width: 40, height: 40, borderRadius: 12,
    justifyContent: 'center', alignItems: 'center', marginRight: 12,
  },
  textoFiltroContainer: { flex: 1 },
  txtLabelFiltro: { fontSize: 10, color: '#888', fontWeight: '700', letterSpacing: 0.5 },
  txtDataAtual: { fontSize: 15, color: '#1E22AA', fontWeight: 'bold' },

  cardResumo: { backgroundColor: "#fff", borderRadius: 12, height: 95, flexDirection: "row", marginBottom: 12, elevation: 3 },
  barraStatus: { width: 10, height: "100%", borderTopLeftRadius: 12, borderBottomLeftRadius: 12 },
  cardInfo: { flex: 1, paddingHorizontal: 15, justifyContent: "center" },
  cardTitulo: { fontSize: 12, color: "#666", fontWeight: "500" },
  cardValor: { fontSize: 20, fontWeight: "bold", color: "#333" },
  
  containerSubtitulo: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center' },
  cardSubtitulo: { fontSize: 11 },
  cardSubtituloBase: { color: "#999" },
  txtPeriodoLinha: { fontSize: 10, color: "#bbb", fontWeight: 'normal' },

  botaoTrocaCard: { padding: 4, borderRadius: 8, backgroundColor: "#f0f0ff"},
  ...
  itemRanking: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: "#f2f2f2" },
  nomeItem: { fontSize: 14, color: "#333", flex: 1 },
  qtdItem: { fontSize: 14, fontWeight: "bold", color: "#1E22AA" },
